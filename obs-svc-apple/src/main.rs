// obs-svc-apple is the frameless, always-on-top macOS floating widget. It polls
...
// obs_svc_apple library and is unit-tested headlessly. This file is the NSWindow
...
fn main() {
    use obs_svc_apple::{fetch, render::RenderModel};
    let cfg = fetch::Config::from_env();
    eprintln!(
        "obs-svc-apple: no native window on this platform; polling {} once",
        cfg.state_url
    );
...
mod macos {
    ...
    use obs_svc_apple::fetch::{self, Config};
    use obs_svc_apple::render::{colour_for, RenderModel, Rgb};
