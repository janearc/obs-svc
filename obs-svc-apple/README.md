...
This crate is the presentation layer (`obs-svc-apple`) named in the architecture
record (`../observability_architecture_v1.md` §3.3). It lives inside the
`obs-svc` repository but is a standalone Rust crate; it does not touch the Go
`obs-svc-agg` module.
...
launched it (`Ctrl-C`), or `pkill obs-svc-apple`.
